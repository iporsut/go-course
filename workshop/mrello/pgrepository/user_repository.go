...
func (r *userRepository) FindUserByEmail(ctx context.Context, email string) (*mrello.User, error) {
	row := r.db.QueryRowContext(ctx, `SELECT user_id, email, password, created_at, updated_at FROM users WHERE email = $1`, email)
...
func (r *userRepository) CreateUser(ctx context.Context, email string, passwordHash string) (*mrello.User, error) {
	row := r.db.QueryRowContext(ctx, `INSERT INTO users (user_id, email, password, created_at, updated_at) VALUES ($1, $2, $3, NOW(), NOW()) RETURNING user_id,  created_at, updated_at`, uuid.NewString(), email, passwordHash)
	var user mrello.User
